...
def receive_msg(msg):
    ...
    for k in msg_information:
        ...
        if int(time.time()) - m_time > 180:
            del_info.append(k)
            if msg_information[k]['msg_type'] == "Attachment" or msg_information[k]['msg_type'] == "Video" or \
                    msg_information[k]['msg_type'] == 'Picture' or msg_information[k]['msg_type'] == 'Recording':
                a = msg_information[k]['msg_content']
                del_img.append(a)
    if del_img:
        for name in del_img:
            os.remove("~/" + name)
