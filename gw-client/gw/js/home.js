		...
		if(res.err_code == 0) {
			...
			for(var i = 0; i < data.length; i++) {
				if(i == 0) {
					...
					var src = data[i].Url;
					$('.lbdian').append(`<li data-target="#carousel-example-generic" data-slide-to="${i}""></li>`);
...
function more() {
	...
	if(end < series_photo_num) {
		for(var i = start; i < end; i++) {
			$('#yz').append(`<div class="col-xs-6 col-md-3 photoshow-item" onclick="btnphoto(${imgs[i].Id})"><img src="${imgs[i].Cover}"/><span class="msg"><p>&nbsp;&nbsp;${imgs[i].Name}</p><p><img style="width: 30px;height: 16px;" src="../img/icon/icon_browse.png"/>&nbsp;${imgs[i].Watch}&nbsp;&nbsp;</p></span></div>`);
		}
...
$('.admsg').append(`<label style='padding: 5px 0;'>公司总部地址:广州天河区金穗路5号佛奥广场4楼412室</label>`);
